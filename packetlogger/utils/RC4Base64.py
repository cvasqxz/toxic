...
class RC4Base64:
	...
	def decipher(self, data, offset=0, length=None):
		if length is None:
			length = len(data)
		buffer = bytearray(data[offset:offset+length])
		result_buffer = io.BytesIO()
		while len(buffer) > 0:
			print(len(buffer))
			first_byte = self.BASE64_DECODING_MAP[buffer.pop()]
			second_byte = self.BASE64_DECODING_MAP[buffer.pop()]
			byte1a = first_byte << 2
			byte1b = (second_byte & 0x30) >> 4
			result_buffer.write(struct.pack('>b', (byte1a | byte1b) ^ self.move_up()))
			buffer = buffer[2:]
			if buffer:
				third_byte = self.BASE64_DECODING_MAP[buffer.pop()]
				byte2a = (second_byte & 0x0F) << 4
				byte2b = (third_byte & 0x3C) >> 2
				result_buffer.write(struct.pack('>b', (byte2a | byte2b) ^ self.move_up()))
				buffer = buffer[1:]
				if buffer:
					fourth_byte = self.BASE64_DECODING_MAP[buffer.pop()]
					byte3a = (third_byte & 0x03) << 6
					byte3b = fourth_byte & 0x3F
					result_buffer.write(struct.pack('>b', (byte3a | byte3b) ^ self.move_up()))
					buffer = buffer[1:]
